...
def get_activity_type_stream(activity):
    ...
    if "primaryAttribute" in activity:
        ...
        mdata = metadata.write(
            mdata, ("properties", "primary_attribute_value"), "inclusion", "automatic"
        )
        mdata = metadata.write(
            mdata, ("properties", "primary_attribute_name"), "inclusion", "automatic"
        )
        mdata = metadata.write(
            mdata,
    ...
    if "attributes" in activity:
        for attr in activity["attributes"]:
            ...
            field_schema, mdata = get_schema_for_type(
                attr["dataType"],
                breadcrumb=("properties", attr_name),
                mdata=mdata,
                null=True,
            )
            if field_schema:
...
def discover_leads(client):
    ...
    for field in data["result"]:
        ...
        if field["rest"]["name"] in LEAD_REQUIRED_FIELDS:
            field_schema, mdata = get_schema_for_type(
                field["dataType"], ("properties", field_name), mdata, null=False
            )
        else:
            field_schema, mdata = get_schema_for_type(
                field["dataType"], ("properties", field_name), mdata, null=True
            )

        if not field_schema:
            singer.log_debug(
                "Marketo type %s unsupported for leads.%s",
                field["dataType"],
                field_name,
            )
            continue
